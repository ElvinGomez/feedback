...
export const TRAVEL_MODE_FEATURE_PATH = 'spots:discovery:countrySwitch';
 ...
 * (same JWT feature path + same global `spots.discovery.countrySwitch` leaf)
...
export async function isTravelModeEnabled(req: Request): Promise<boolean> {
  ...
  const hasPath =
    isAdmin || featurePaths.includes(TRAVEL_MODE_FEATURE_PATH);
  if (!hasPath) {
    return false;
  }
